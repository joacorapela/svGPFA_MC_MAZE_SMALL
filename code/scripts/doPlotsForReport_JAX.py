...
def main(argv):
    ...
    parser.add_argument("--cluster", help="cluster to plot",
                        type=int, default=10)
    parser.add_argument("--trial", help="trial to plot", type=int, default=0)
    ...
    preIntensityFigFilenamePattern = "../../figures/{:08d}_preIntensity_clusterID_{:03d}.{{:s}}".format(estResNumber, cluster)
    orthonormalizedEmbeddingParamsFigFilenamePattern = "../../figures/{:08d}_orthonormalized_preIntensity_params.{{:s}}".format(estResNumber)
#     CIFFigFilenamePattern = "../../figures/{:08d}_CIF_trial{:03d}_cluster_id_{:03d}.{{:s}}".format(estResNumber, trial, cluster)
#     CIFimageFigFilenamePattern = "../../figures/{:08d}_CIFimage_cluster_id_{:03d}_sortedBy{:s}.{{:s}}".format(estResNumber, cluster, column_name)
    CIFsOneClusterAllTrialsFigFilenamePattern = "../../figures/{:08d}_CIF_cluster{:03d}_allTrials.{{:s}}".format(estResNumber, cluster)
    ksTestTimeRescalingNumericalCorrectionFigFilenamePattern = "../../figures/{:08d}_ksTestTimeRescaling_numericalCorrection_trial{:03d}_cluster_id_{:d}.{{:s}}".format(estResNumber, trial, cluster)
    rocFigFilenamePattern = "../../figures/{:08d}_predictive_analysis_trial{:03d}_cluster_id_{:d}.{{:s}}".format(estResNumber, trial, cluster)
    ...
    assert(len(clusters) == len(spikes_times[0]))
...
#         trials_annotations=trials_annotations,
    ...
    # calculate expected CIF mean and std
    # cif_means, cifstds \in n_trials x n_clusters x n_times
    cif_means, cif_stds = svGPFA.utils.statsUtils.computeCIFmeanAndSTD(
        h_means=h_means, h_vars=h_vars)
    # cif_values_GOF = cif_values[trial, cluster_index, :]

    # CIF

    # CIFs one cluster all trials
    ...
    fig.write_image(CIFsOneClusterAllTrialsFigFilenamePattern.format("png"))
    fig.write_html(CIFsOneClusterAllTrialsFigFilenamePattern.format("html"))

    """
    trial_times_GOF = times[trial, :, 0]
    spikes_times_GOF = spikes_times[trial][cluster_index]
    title = "Trial {:d}, Cluster {:d} ({:d} spikes)".format(
        trial, cluster, len(spikes_times_GOF))

    # plot KS test time rescaling (numerical correction)
    if len(spikes_times_GOF) > 0:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            diffECDFsX, diffECDFsY, estECDFx, estECDFy, simECDFx, simECDFy, cb = gcnu_common.stats.pointProcesses.tests.KSTestTimeRescalingNumericalCorrection(spikes_times=spikes_times_GOF, cif_times=np.array(trial_times_GOF), cif_values=np.array(cif_values_GOF), gamma=ksTestGamma)
    ...
    hovertemplate = "value: %{y}<br>" + \
                    "cluster index: %{x}<br>" + \
                    "%{text}"
    text = [f"cluster_id: {cluster_id}" for cluster_id in clusters]
    # estimatedC, estimatedD = model.getSVEmbeddingParams()
    fig = svGPFA.plot.plotUtilsPlotly.getPlotOrthonormalizedPreIntensityParams(
        C=C, d=d, hovertemplate=hovertemplate, text=text)
    fig.write_image(
